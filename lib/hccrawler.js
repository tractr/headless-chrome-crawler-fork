...
class HCCrawler extends EventEmitter {
  ...
  async _startRequest(options, depth, previousUrl) {
    const { skip, reason, why } = await this._skipRequest(options);
    if (skip) {
      this.emit(HCCrawler.Events.RequestSkipped, { options, reason, why });
  ...
  async _skipRequest(options) {
    const _checkAllowedDomains = this._checkAllowedDomains(options);
    const { allowedDomain } = _checkAllowedDomains;
    if (!allowedDomain) return { skip: true, reason: 'allowedDomain', why: _checkAllowedDomains.why };

    const _checkRequested = await this._checkRequested(options);
    const { requested, obj } = _checkRequested;
    if (requested) {
      return {
        skip: true, reason: 'requested', why: _checkRequested.why, obj,
      };
    }

    const shouldRequest = await this._shouldRequest(options);
    if (!shouldRequest) return { skip: true, reason: 'shouldRequest', why: 'preRequest blocked the request' };

    return { skip: false, reason: '', why: '' };
  }
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    ...
    try {
      ...
      this.emit(HCCrawler.Events.RequestFinished, Object.assign({}, options, {
        headers: { response: res.headers(), request: res.request().headers() },
      }));
      ...
      await crawler.close();
      const update = { options, depth, previousUrl };
      if (res && res.headers() && res.request().headers()) {
        update.headers = { response: res.headers(), request: res.request().headers() };
      }
      extend(error, update);
      const noRetry = this._options.retryDisabledOnErrorTypes
        .some(errType => error.message.includes(errType));
      if (noRetry || retryCount >= options.retryCount) {
  ...
  _checkAllowedDomains(options) {
    ...
    if (options.deniedDomains && checkDomainMatch(options.deniedDomains, hostname)) {
      return { allowedDomain: false, why: `hostname ${hostname} is present in deniedDomains` };
    }
    if (options.allowedDomains && !checkDomainMatch(options.allowedDomains, hostname)) {
      return { allowedDomain: false, why: `hostname ${hostname} is not present in allowedDomains` };
    }
    return { allowedDomain: true, why: '' };
  }
  ...
  async _checkRequested(options) {
    ...
    const { key, obj } = generateKey(options);
    const value = await this._cache.get(key);
    return { requested: !!value, why: `cache key ${key} is already present`, obj };
  }
  ...
  async _followLinks(urls, options, depth) {
    ...
    await Promise.all(map(urls, async url => {
      ...
      const { skip, reason, why } = await this._skipRequest(_options);
      if (skip) {
        this.emit(HCCrawler.Events.RequestSkipped, { options, reason, why });
        return;
      }
