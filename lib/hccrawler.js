...
class HCCrawler extends EventEmitter {
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    ...
    try {
      ...
      await crawler.close();
      extend(error, { options, depth, previousUrl }, { message: (error.message || 'no message...'), name: error.name, stack: (error.stack || 'no stack...') });
      const noRetry = this._options.retryDisabledOnErrorTypes
        .some(errType => error.message.includes(errType));

      console.log(`noRetry : ${noRetry}, error.message ${error.message}`);
