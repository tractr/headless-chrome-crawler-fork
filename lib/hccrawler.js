...
class HCCrawler extends EventEmitter {
  ...
  async _skipRequest(options) {
    ...
    const { requested } = _checkRequested;
    if (requested) return { skip: true, reason: 'requested', why: _checkRequested.why };
  ...
  async _checkRequested(options) {
    ...
    return { requested: !!value, why: `cache key ${key} is already present` };
  }
