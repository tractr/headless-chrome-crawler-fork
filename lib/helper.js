...
class Helper {
   ...
   */
  static hash(src) {
    const md5hash = crypto.createHash('sha3-512');
  ...
  static generateKey(options) {
    const obj = pick(options, PICKED_OPTION_FIELDS);
    const json = stringify(sortKeysRecursive(obj));
    return { key: Helper.hash(json).substring(0, MAX_KEY_LENGTH), obj };
  }
