...
class Crawler {
  ...
  async crawl() {
    ...
    if (contentLength < 100) {
      throw new Error(`Content length is too small: ${contentLength} < 100`);
    }
    */
    await this._waitFor();
    const [
   ...
   */
  _handlePageEvents() {
    this._page.on('pageerror', err => void debugConsole(err.message));
  ...
  async _request() {
    ...
    let fullResponse = await this._page.goto(this._options.url, gotoOptions);
    if (fullResponse === null) {
      fullResponse = await this._page.waitForResponse(() => true);
    }
  ...
  async _collectLinks(baseUrl) {
    const links = await this._page.evaluate(() => {
      const results = [];
    ...
    const resolvedLinks = [];
    links.forEach(link => {
      const resolvedLink = resolveUrl(link, baseUrl);
      if (resolvedLink) resolvedLinks.push(resolvedLink);
    });
    return uniq(resolvedLinks);
  }
